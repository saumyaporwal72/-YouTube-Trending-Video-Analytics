...
drop table youtube_data;
CREATE TABLE youtube_data (
    video_id VARCHAR(50),
    video_title VARCHAR(255),
    channel_name VARCHAR(255),
    category VARCHAR(100),
    publish_date VARCHAR(50),   -- kept as TEXT
    views BIGINT,
    likes BIGINT,
    comments BIGINT,
    duration_seconds INT,
    country VARCHAR(100),
    is_trending VARCHAR(10),
    engagement_rate FLOAT
);
ALTER TABLE youtube_data
ADD COLUMN publish_date_new DATE;
 Set SQL_SAFE_UPDATES =0;

UPDATE youtube_data
SET publish_date_new = STR_TO_DATE(publish_date, '%m/%d/%Y');

ALTER TABLE youtube_data DROP COLUMN publish_date;
ALTER TABLE youtube_data 
-- CHANGE publish_date_new publish_date DATE;
...
-- this is changing my date varchar in to date format
describe youtube_data;
UPDATE youtube_data
SET publish_date = STR_TO_DATE(publish_date, '%m/%d/%Y %H:%i');
ALTER TABLE youtube_data
MODIFY publish_date DATETIME;
describe youtube_data;
